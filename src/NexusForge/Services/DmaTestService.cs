...
public class DmaTestService
{
    ...
    /// and writes mmap.txt + memmap.txt to every path in <paramref name="outputPaths"/>.
    /// The generated file prevents the AMD/Intel MMIO-hole wedge on all DMA tools
    /// (pcileech, memprocfs, radars) that support -memmap or auto-load mmap.txt.
    /// </summary>
    public Task<MmapResult> GenerateMmapAsync(
        IEnumerable<string> outputPaths,
        IProgress<FlashProgress>? progress,
        CancellationToken ct) =>
        Task.Run(() => GenerateMmap(outputPaths.ToList(), progress, ct), ct);
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            progress?.Report(Prog("Reading", 30, "Reading physical memory map from connected PC..."));
            if (!VmmNative.VMMDLL_Map_GetPhysMem(hVMM, out pMap) || pMap == IntPtr.Zero)
                throw new InvalidOperationException("Failed to retrieve physical memory map from connected PC.");
            ...
            var lines = new List<string>
            {
                ...
                "# Place this file (mmap.txt / memmap.txt) in your radar tool directory",
                "# alongside leechcore.dll. Required on AMD platforms to prevent MMIO-hole",
                "# link wedge. Also required on Intel with IOMMU/VBS active.",
                "# Re-generate if you change the target PC hardware or RAM configuration.",
                ""
            };
            ...
            _log.Info($"Memory map: {cMap} regions, {result.TotalRamGb:F1} GB total RAM");
            progress?.Report(Prog("Writing", 70, $"Writing mmap to {outputPaths.Count} location(s)..."));

            var written  = new List<string>();
            var skipped  = new List<string>();

            foreach (var dir in outputPaths)
            {
                ct.ThrowIfCancellationRequested();
                try
                {
                    if (!Directory.Exists(dir)) { skipped.Add(dir); continue; }
                    foreach (var name in new[] { "mmap.txt", "memmap.txt" })
                    {
                ...
                {
                    _log.Warn($"Could not write mmap to {dir}: {ex.Message}");
                    skipped.Add(dir);
                }
            }
            ...
            result.Success      = written.Count > 0;

            _log.Info($"mmap.txt written to {written.Count} location(s).");
        }
        catch (Exception ex)
        {
            result.ErrorMessage = SanitizeError(ex.Message);
        ...
        finally
        {
            ...
            progress?.Report(Prog(
                ...
                result.Success ? 100 : 0,
                result.Success
                    ? $"Memory map written ({result.RegionCount} regions, {result.TotalRamGb:F1} GB)"
                    : result.ErrorMessage));
        }
        return result;
    }
